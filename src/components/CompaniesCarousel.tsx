...
const CompaniesCarousel = () => {
  return <div className="w-full px-12 py-8">
      <Carousel opts={{
      align: "start",
      loop: true
    }} className="w-full">
        <CarouselContent>
          {companies.map((company, index) => <CarouselItem key={index} className="md:basis-1/2 lg:basis-1/3">
              <div className="p-1">
                <div className="overflow-hidden rounded-full aspect-square bg-white/10 backdrop-blur-sm">
                  <img src={company.image} alt={company.name} className="h-full w-full p-4 rounded-full transition-transform duration-300 hover:scale-105 object-contain" />
                </div>
              </div>
            </CarouselItem>)}
        </CarouselContent>
